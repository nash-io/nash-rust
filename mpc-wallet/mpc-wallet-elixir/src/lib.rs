...
rustler_export_nifs! {
    ...
    ("get_rpool_size", 1, get_rpool_size),
    ("dh_init", 2, dh_init),
...
fn generate_paillier_keypair_and_proof<'a>(env: Env<'a>, _args: &[Term<'a>]) -> Term<'a> {
...
/// input: client_dh_publics, curve (Secp256r1, Secp256k1, of Curve25519)
/// output: rpool_new, server_dh_publics
fn dh_rpool<'a>(env: Env<'a>, args: &[Term<'a>]) -> Term<'a> {
    let client_dh_publics_str: String = match args[0].decode() {
        Ok(v) => v,
        Err(_) => return (atoms::error(), &"error parsing client_dh_publics").encode(env),
    };
    let curve_str: String = match args[1].decode() {
        Ok(v) => v,
        Err(_) => return (atoms::error(), &"error parsing curve").encode(env),
    };
    let curve: common::Curve = match serde_json::from_str(&curve_str) {
        Ok(v) => v,
        Err(_) => return (atoms::error(), &"error deserializing curve").encode(env),
    };
    ...
    if curve == common::Curve::Secp256k1 {
        let client_dh_publics: Vec<Secp256k1Point> =
            match serde_json::from_str(&client_dh_publics_str) {
                Ok(v) => v,
                Err(_) => {
                    return (atoms::error(), &"error deserializing client_dh_publics").encode(env)
                }
            };
        let (server_dh_secrets, server_dh_publics) =
            match common::dh_init_secp256k1(client_dh_publics.len()) {
                Ok(v) => v,
                Err(_) => return (atoms::error(), &"error: n too big").encode(env),
            };
        let rpool_new =
        ...
        return (atoms::error(), &"error: invalid curve").encode(env);
    }
    (atoms::ok(), &rpool_new_json, &server_dh_publics_json).encode(env)
}

/// finalize presignature to normal ECDSA signature
/// input: paillier_sk, presig, r, k, curve, pubkey, msg_hash
/// output: r, s, recid
fn complete_sig_ecdsa<'a>(env: Env<'a>, args: &[Term<'a>]) -> Term<'a> {
    let paillier_sk_str: String = match args[0].decode() {
        Ok(v) => v,
        Err(_) => return (atoms::error(), &"error parsing paillier_sk").encode(env),
    };
    let paillier_sk: DecryptionKey = match serde_json::from_str(&paillier_sk_str) {
        Ok(v) => v,
        Err(_) => return (atoms::error(), &"error deserializing paillier_sk").encode(env),
    };
    let presig_str: String = match args[1].decode() {
        Ok(v) => v,
        Err(_) => return (atoms::error(), &"error parsing presig").encode(env),
    };
    let presig = match BigInt::from_hex(&presig_str) {
        Ok(v) => v,
        Err(_) => return (atoms::error(), &"error deserializing presig").encode(env),
    };
    let r_str: String = match args[2].decode() {
        Ok(v) => v,
        Err(_) => return (atoms::error(), &"error parsing r").encode(env),
    };
    let r = match BigInt::from_hex(&r_str) {
        Ok(v) => v,
        Err(_) => return (atoms::error(), &"error deserializing r").encode(env),
    };
    let k_str: String = match args[3].decode() {
        Ok(v) => v,
        Err(_) => return (atoms::error(), &"error parsing k").encode(env),
    };
    let k = match BigInt::from_hex(&k_str) {
        Ok(v) => v,
        Err(_) => return (atoms::error(), &"error deserializing k").encode(env),
    };
    ...
    let pubkey: String = match args[5].decode() {
        Ok(v) => v,
        Err(_) => return (atoms::error(), &"error parsing pubkey").encode(env),
    };
    let msg_hash_str: String = match args[6].decode() {
        Ok(v) => v,
        Err(_) => return (atoms::error(), &"error parsing msg_hash").encode(env),
    };
    let msg_hash = match BigInt::from_hex(&msg_hash_str) {
        Ok(v) => v,
        Err(_) => return (atoms::error(), &"error deserializing msg_hash").encode(env),
    };

    let (r, s, recid) = match server::complete_sig_ecdsa(
        &paillier_sk,
        &presig,
        &r,
        &k,
        ...
        Ok(v) => v,
        Err(_) => return (atoms::error(), &"error: completing signature failed").encode(env),
    };
...
/// output: r, s
fn complete_sig_eddsa<'a>(env: Env<'a>, args: &[Term<'a>]) -> Term<'a> {
    let server_secret_share_str: String = match args[0].decode() {
        Ok(v) => v,
        Err(_) => return (atoms::error(), &"error parsing server_secret_share").encode(env),
    };
    let server_secret_share = match BigInt::from_hex(&server_secret_share_str) {
        Ok(v) => v,
        Err(_) => return (atoms::error(), &"error deserializing server_secret_share").encode(env),
    };
    let presig_str: String = match args[1].decode() {
        Ok(v) => v,
        Err(_) => return (atoms::error(), &"error parsing presig").encode(env),
    ...
    let presig = match BigInt::from_hex(&presig_str) {
        Ok(v) => v,
        Err(_) => return (atoms::error(), &"error deserializing presig_str").encode(env),
    };
    let r_str: String = match args[2].decode() {
        Ok(v) => v,
        Err(_) => return (atoms::error(), &"error parsing r").encode(env),
    ...
    let r_server_str: String = match args[3].decode() {
        Ok(v) => v,
        Err(_) => return (atoms::error(), &"error parsing r_server").encode(env),
    };
    let r_server_int = match BigInt::from_hex(&r_server_str) {
        Ok(v) => v,
        Err(_) => return (atoms::error(), &"error deserializing r_server_str").encode(env),
    };
    let r_server: Ed25519Scalar = match ECScalar::from(&r_server_int) {
        Ok(v) => v,
        Err(_) => return (atoms::error(), &"error deserializing r_server").encode(env),
    };
    ...
    let msg_str: String = match args[5].decode() {
        Ok(v) => v,
        Err(_) => return (atoms::error(), &"error parsing msg").encode(env),
    };
    let msg = match BigInt::from_hex(&msg_str) {
        Ok(v) => v,
        Err(_) => return (atoms::error(), &"error deserializing msg").encode(env),
    };
...
/// output: ok|error
fn verify<'a>(env: Env<'a>, args: &[Term<'a>]) -> Term<'a> {
    ...
    let s_str: String = match args[1].decode() {
        Ok(v) => v,
        Err(_) => return (atoms::error(), &"error parsing s").encode(env),
    };
    let s = match BigInt::from_hex(&s_str) {
        Ok(v) => v,
        Err(_) => return (atoms::error(), &"error deserializing s").encode(env),
    };
    ...
    let msg_hash_str: String = match args[3].decode() {
        Ok(v) => v,
        Err(_) => return (atoms::error(), &"error parsing msg_hash / msg").encode(env),
    };
    let msg_hash = match BigInt::from_hex(&msg_hash_str) {
        Ok(v) => v,
        Err(_) => return (atoms::error(), &"error deserializing msg_hash").encode(env),
...
/// input: n (number of key pairs to generate), curve (Secp256r1, Secp256k1, or Curve25519)
/// output: dh_secrets, dh_publics
fn dh_init<'a>(env: Env<'a>, args: &[Term<'a>]) -> Term<'a> {
    let n: usize = match args[0].decode() {
        Ok(v) => v,
        Err(_) => return (atoms::error(), &"error parsing n").encode(env),
    };
    let curve_str: String = match args[1].decode() {
        Ok(v) => v,
        Err(_) => return (atoms::error(), &"error parsing curve").encode(env),
    ...
    (atoms::ok(), &dh_secrets_json, &dh_publics_json).encode(env)
}
...
fn compute_presig<'a>(env: Env<'a>, args: &[Term<'a>]) -> Term<'a> {
    let api_childkey_str: String = match args[0].decode() {
        Ok(v) => v,
        Err(_) => return (atoms::error(), &"error parsing api_childkey").encode(env),
    };
    let api_childkey: client::APIchildkey = match serde_json::from_str(&api_childkey_str) {
        Ok(v) => v,
        Err(_) => return (atoms::error(), &"error deserializing api_childkey").encode(env),
    };
    ...
    let (presig, r) = match client::compute_presig(&api_childkey, &msg_hash, curve) {
        Ok(v) => v,
        Err(_) => return (atoms::error(), &"error getting value from rpool").encode(env),
    };
    if curve == common::Curve::Secp256k1 || curve == common::Curve::Secp256r1 {
        ...
        (atoms::error(), &"invalid curve").encode(env)
    }
}
...
/// output: ok|error
fn fill_rpool<'a>(env: Env<'a>, args: &[Term<'a>]) -> Term<'a> {
    let server_dh_secrets_str: String = match args[0].decode() {
        Ok(v) => v,
        Err(_) => return (atoms::error(), &"error parsing server_dh_secrets").encode(env),
    };
    ...
    if curve == common::Curve::Secp256k1 {
        let paillier_pk_str: String = match args[3].decode() {
            Ok(v) => v,
            Err(_) => return (atoms::error(), &"error parsing paillier_pk").encode(env),
        };
        let paillier_pk: EncryptionKey = match serde_json::from_str(&paillier_pk_str) {
            Ok(v) => v,
            Err(_) => return (atoms::error(), &"error deserializing paillier_pk").encode(env),
        };
        let server_dh_secrets: Vec<Secp256k1Scalar> =
            match serde_json::from_str(&server_dh_secrets_str) {
                Ok(v) => v,
                Err(_) => {
                    return (atoms::error(), &"error deserializing server_dh_secrets").encode(env)
                }
            };
        let client_dh_publics: Vec<Secp256k1Point> =
            match serde_json::from_str(&client_dh_publics_str) {
        ...
        match client::fill_rpool_secp256k1(server_dh_secrets, &client_dh_publics, &paillier_pk) {
            Ok(v) => v,
            Err(_) => return (atoms::error(), &"error filling rpool").encode(env),
        };
    } else if curve == common::Curve::Secp256r1 {
        let paillier_pk_str: String = match args[3].decode() {
            Ok(v) => v,
...
/// retrieve number of values in a specific rpool
/// input: curve
/// output: number of values
fn get_rpool_size<'a>(env: Env<'a>, args: &[Term<'a>]) -> Term<'a> {
    let curve_str: String = match args[0].decode() {
        Ok(v) => v,
        Err(_) => return (atoms::error(), &"error parsing curve").encode(env),
    };
    let curve: common::Curve = match serde_json::from_str(&curve_str) {
    ...
    let size = match client::get_rpool_size(curve) {
        Ok(v) => v,
        Err(_) => return (atoms::error(), &"error: invalid curve").encode(env),
    };
    let size_json = serde_json::to_string(&size).unwrap();
    (atoms::ok(), &size_json).encode(env)
}

/// initialize API child key creation by setting the full secret key
/// input: secret_key
/// output: api_childkey_creator
fn init_api_childkey_creator<'a>(env: Env<'a>, args: &[Term<'a>]) -> Term<'a> {
    let secret_key_str: String = match args[0].decode() {
        Ok(v) => v,
        Err(_) => return (atoms::error(), &"error parsing secret_key").encode(env),
    };
    let secret_key = match BigInt::from_hex(&secret_key_str) {
        Ok(v) => v,
        Err(_) => return (atoms::error(), &"error deserializing secret_key").encode(env),
    };
    ...
    (atoms::ok(), &api_childkey_creator_json).encode(env)
}
...
/// output: api_childkey_creator
fn verify_paillier<'a>(env: Env<'a>, args: &[Term<'a>]) -> Term<'a> {
    let api_childkey_creator_str: String = match args[0].decode() {
        Ok(v) => v,
        Err(_) => return (atoms::error(), &"error parsing api_childkey_creator").encode(env),
    };
    let api_childkey_creator: client::APIchildkeyCreator =
        match serde_json::from_str(&api_childkey_creator_str) {
            Ok(v) => v,
            Err(_) => {
                return (atoms::error(), &"error deserializing api_childkey_creator").encode(env)
            }
        };
    let paillier_pk_str: String = match args[1].decode() {
        Ok(v) => v,
    ...
    let correct_key_proof_str: String = match args[2].decode() {
        Ok(v) => v,
        Err(_) => return (atoms::error(), &"error parsing correct_key_proof").encode(env),
    };
    let correct_key_proof: common::CorrectKeyProof =
        match serde_json::from_str(&correct_key_proof_str) {
            Ok(v) => v,
            Err(_) => {
                return (atoms::error(), &"error deserializing correct_key_proof").encode(env)
            }
        };
    ...
    let api_childkey_creator_new =
        match api_childkey_creator.verify_paillier(&paillier_pk, &correct_key_proof) {
            Ok(v) => v,
            Err(_) => return (atoms::error(), &"error verifying paillier_pk").encode(env),
        };
    let api_childkey_creator_new_json = serde_json::to_string(&api_childkey_creator_new).unwrap();
    (atoms::ok(), &api_childkey_creator_new_json).encode(env)
}

/// create API childkey
/// input: api_childkey_creator, curve
/// output: api_childkey
fn create_api_childkey<'a>(env: Env<'a>, args: &[Term<'a>]) -> Term<'a> {
    let api_childkey_creator_str: String = match args[0].decode() {
        Ok(v) => v,
        Err(_) => return (atoms::error(), &"error parsing api_childkey_creator").encode(env),
    };
    ...
    let api_childkey = match api_childkey_creator.create_api_childkey(curve) {
        Ok(v) => v,
        Err(_) => return (atoms::error(), &"error: paillier_pk not verified yet").encode(env),
    };
    let api_childkey_json = serde_json::to_string(&api_childkey).unwrap();
    (atoms::ok(), &api_childkey_json).encode(env)
}

/// Derive public key from given secret key.
/// input: (full) secret key, curve
/// output: public_key
fn publickey_from_secretkey<'a>(env: Env<'a>, args: &[Term<'a>]) -> Term<'a> {
    let secret_key_str: String = match args[0].decode() {
        Ok(v) => v,
        Err(_) => return (atoms::error(), &"error parsing secret_key").encode(env),
    };
    ...
    let public_key = match common::publickey_from_secretkey(&secret_key, curve) {
        Ok(v) => v,
        Err(_) => {
            return (atoms::error(), &"error deriving public key from secret key").encode(env)
        }
    };
    let public_key_json = serde_json::to_string(&public_key).unwrap();
    (atoms::ok(), &public_key_json).encode(env)
}

/// decrypt server secret share (or any other ciphertext) using Paillier
fn decrypt<'a>(env: Env<'a>, args: &[Term<'a>]) -> Term<'a> {
    let paillier_sk_str: String = match args[0].decode() {
        Ok(v) => v,
        Err(_) => return (atoms::error(), &"error parsing paillier_sk").encode(env),
    };
    ...
    let ciphertext_str: String = match args[1].decode() {
        Ok(v) => v,
        Err(_) => return (atoms::error(), &"error parsing ciphertext").encode(env),
    };
    let ciphertext = match BigInt::from_hex(&ciphertext_str) {
        Ok(v) => v,
        Err(_) => return (atoms::error(), &"error deserializing ciphertext").encode(env),
    };
    let cleartext = server::decrypt(&paillier_sk, &ciphertext);
    let cleartext_json = serde_json::to_string(&cleartext).unwrap();
    (atoms::ok(), &cleartext_json).encode(env)
}
