...
fn dh_init_export(mut cx: FunctionContext) -> JsResult<JsString> {
    ...
    let result = dh_init(n, curve);
    Ok(cx.string(result))
}

fn fill_rpool_export(mut cx: FunctionContext) -> JsResult<JsString> {
    let client_dh_secrets_str = cx.argument::<JsString>(0)?.value(&mut cx);
    let server_dh_publics_str = cx.argument::<JsString>(1)?.value(&mut cx);
    // FIXME: The argument order is somehow wrong. We need to swap 2 and 3 to make this function work.
    let curve = get_curve(&mut cx, 3)?;
    let paillier_pk_str = cx.argument::<JsString>(2)?.value(&mut cx);
    let result = fill_rpool(client_dh_secrets_str, server_dh_publics_str, curve, paillier_pk_str);
    Ok(cx.string(result))
}

fn get_rpool_size_export(mut cx: FunctionContext) -> JsResult<JsString> {
    let curve = get_curve(&mut cx, 0)?;
    let result = get_rpool_size(curve);
    Ok(cx.string(result))
}

fn compute_presig_export(mut cx: FunctionContext) -> JsResult<JsString> {
    let api_childkey_str = cx.argument::<JsString>(0)?.value(&mut cx);
    let msg_hash_str = cx.argument::<JsString>(1)?.value(&mut cx);
    let curve = get_curve(&mut cx, 2)?;
    let result = compute_presig(api_childkey_str, msg_hash_str, curve);
    Ok(cx.string(result))
}
