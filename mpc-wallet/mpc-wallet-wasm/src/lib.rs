...
/// Initialize ECDSA API key creation by setting the full secret key
...
/// Initialize ECDSA API key creation by setting the full secret key and the paillier public key.
...
#[wasm_bindgen]
pub fn create_ecdsa_api_childkey(api_childkey_creator_str: &str, curve_str: &str) -> String {
    let api_childkey_creator: client::APIchildkeyCreator =
        match serde_json::from_str(&api_childkey_creator_str) {
            Ok(v) => v,
            Err(_) => {
...
/// Create EdDSA API childkey
/// Input: secret_key: secret key
/// Output: api_eddsa_childkey: EdDSA API childkey struct, server_secret_share: server's secret share
#[wasm_bindgen]
pub fn create_eddsa_api_childkey(secret_key_str: &str) -> String {
    let secret_key = match BigInt::from_hex(&secret_key_str) {
        Ok(v) => v,
        Err(_) => {
            return serde_json::to_string(&(false, &"error deserializing secret_key")).unwrap()
    ...
    let (api_childkey, server_secret_share) = match client::create_eddsa_api_childkey(&secret_key) {
        Ok(v) => v,
        Err(_) => {
            return serde_json::to_string(&(false, &"error: creating eddsa api child key failed")).unwrap()
        }
    };
    serde_json::to_string(&(true, &api_childkey, &server_secret_share)).unwrap()
}
...
#[wasm_bindgen]
pub fn compute_presig_ecdsa(api_childkey_str: &str, msg_hash_str: &str, curve_str: &str) -> String {
...
/// Input: api_childkey: EdDSA API childkey struct, msg: message
/// Output: r: message-independent part of the signature used, s_client: presignature
#[wasm_bindgen]
pub fn compute_presig_eddsa(api_childkey_eddsa_str: &str, msg_str: &str) -> String {
    let api_childkey: client::APIchildkeyEdDSA = match serde_json::from_str(&api_childkey_eddsa_str) {
        Ok(v) => v,
        Err(_) => {
            return serde_json::to_string(&(false, &"error deserializing api_childkey")).unwrap()
        }
    ...
    let msg = match BigInt::from_hex(&msg_str) {
        Ok(v) => v,
        Err(_) => return serde_json::to_string(&(false, &"error deserializing msg")).unwrap(),
    };
    let (r, s_client) = match client::compute_presig_eddsa(&api_childkey, &msg) {
        Ok(v) => v,
        Err(_) => {
            return serde_json::to_string(&(
                false,
                &"error: computing presignature failed. rpool empty?",
            ))
            .unwrap()
        }
    };
    ...
    serde_json::to_string(&(
        &true,
        &format!("{:0>64}", s_client.to_bigint().to_hex()),
        &r.to_hex(),
    ))
    .unwrap()
}
...
pub fn verify(
    ...
    let msg_hash = match BigInt::from_hex(&msg_hash_str) {
        Ok(v) => v,
        Err(_) => return serde_json::to_string(&(false, &"error deserializing msg_hash / msg")).unwrap(),
    };
    let curve: common::Curve = match serde_json::from_str(&curve_str) {
        Ok(v) => v,
        Err(_) => return serde_json::to_string(&(false, &"error deserializing curve")).unwrap(),
...
mod tests {
    ...
    fn test_compute_presig_k1_ok() {
        ...
        let result = compute_presig_ecdsa(
