...
use crate::common::{
    correct_key_proof_rho, eddsa_s_hash, eddsa_signingkey_from_secretkey, publickey_from_secretkey, CorrectKeyProof, Curve, CORRECT_KEY_M,
};
...
impl APIchildkeyCreator {
    ...
    pub fn create_api_childkey(self, curve: Curve) -> Result<APIchildkey, ()> {
        ...
        // keys to some untrusted device without risking to leak secret key material.            
            ...
            // Optimization: we don't need paillier_pk at all, and server_secret_share_encrypted does not need to be part of the api key
            Ok(APIchildkey {
                paillier_pk: self.paillier_pk.unwrap(),
                public_key,
                client_secret_share: client_secret_share.to_bigint(),
...
pub fn compute_presig(
    ...
    if curve == Curve::Secp256k1 {
        ...
        let hash: Ed25519Scalar = eddsa_s_hash(&r, &pk, msg_hash)?;
        ...
        tmp.zeroize();
        r_client.zeroize();
        return Ok((s_client.to_bigint(), r.to_bigint()));
    ...
    let c3 = compute_ecdsa_presig_curveindependent(api_childkey, msg_hash, &rx, &q, &k, &rn);
    k.zeroize_bn();
    rn.zeroize_bn();
    Ok((c3, r))
}
...
lazy_static! {
    ...
    static ref RPOOL_CURVE25519:SegQueue<(BigInt, (DateTime<Utc>, BigInt))> = SegQueue::new();
...
pub fn fill_rpool_curve25519(
    ...
    for i in 0..own_dh_secrets.len() {
        ...
        if let Ok(r) = own_dh_public + other_dh_publics[i] {
            RPOOL_CURVE25519.push((r.to_bigint(), (Utc::now(),own_dh_secrets[i].to_bigint())))
        };
    }
    #[cfg(not(feature = "wasm"))]
    (0..own_dh_secrets.len()).into_par_iter().for_each(|i| {
...
pub fn get_rpool_size(curve: Curve) -> Result<usize, ()> {
    if curve == Curve::Secp256k1 {
        ...
        // remove all entries that are older than 48 hours. The server expires values after 72 hours, so 24 hours safety margin should be fine.
        for _ in 0..RPOOL_CURVE25519.len() {
            let pool_entry = match RPOOL_CURVE25519.pop() {
                Option::Some(val) => val,
                Option::None => continue,
            };
            if Utc::now() - pool_entry.1.0 < Duration::hours(48) {
                RPOOL_CURVE25519.push(pool_entry);
            }
        }
...
mod tests {
    ...
    use crate::client::{
        compute_presig, encrypt_secret_share, fill_rpool_secp256k1, fill_rpool_secp256r1, fill_rpool_curve25519,
        get_rpool_size, verify_correct_key_proof, APIchildkeyCreator,
    };
    use crate::common::{CorrectKeyProof, Curve, dh_init_curve25519};
    use crate::curves::curve25519::{Ed25519Point, Ed25519Scalar};
