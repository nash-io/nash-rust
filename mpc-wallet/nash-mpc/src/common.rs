...
pub fn verify_eddsa(r_int: &BigInt, s_int: &BigInt, pk_str: &str, msg: &BigInt) -> bool {
    let r = match Ed25519Point::from_bigint(r_int) {
        Ok(v) => v,
        Err(_) => return false,
    };
    let s_bytes: [u8; 32] = match s_int.to_bytes().try_into() {
        Ok(v) => v,
        Err(_) => return false,
    };
...
mod tests {
    ...
    fn test_verify_ed_ok() {
        let r = BigInt::from_hex("92a009a9f0d4cab8720e820b5f642540a2b27b5416503f8fb3762223ebdb69da").unwrap();
        let s = BigInt::from_hex("085ac1e43e15996e458f3613d0f11d8c387b2eaeb4302aeeb00d291612bb0c00").unwrap();
        ...
        let r = BigInt::from_hex("6291d657deec24024827e69c3abe01a30ce548a284743a445e3680d7db5ac3ac").unwrap();
        let s = BigInt::from_hex("18ff9b538d16f290ae67f760984dc6594a7c15e9716ed28dc027beceea1ec40a").unwrap();
        let pk_str = "fc51cd8e6218a1a38da47ed00230f0580816ed13ba3303ac5deb911548908025".to_string();
        let msg = BigInt::from_hex("af82").unwrap();
        assert!(verify_eddsa(&r, &s, &pk_str, &msg));

        let r = BigInt::from_hex("0aab4c900501b3e24d7cdf4663326a3a87df5e4843b2cbdb67cbf6e460fec350").unwrap();
        let s = BigInt::from_hex("aa5371b1508f9f4528ecea23c436d94b5e8fcd4f681e30a6ac00a9704a188a03").unwrap();
