...
pub fn dh_init_curve25519(
    n: usize,
) -> Result<(Vec<Ed25519Scalar>, Vec<Ed25519Point>), ()> {
    // don't allow creating too many values at once.
    if n > 100 {
        return Err(());
    }
...
pub fn verify(r: &BigInt, s: &BigInt, pubkey_str: &str, msg_hash: &BigInt, curve: Curve) -> bool {
    ...
    if curve == Curve::Secp256k1 {
        ...
        let r_point = Ed25519Point::from_bigint(r).unwrap();
        let s_bytes: [u8; 32] = match s.to_bytes().try_into() {
            Ok(v) => v,
            Err(_) => return false,
        };
        let s_ = Ed25519Scalar::from_bytes32(s_bytes).unwrap();
        let pk = Ed25519Point::from_hex(pubkey_str).unwrap();
        if r_point.ge.is_small_order() || pk.ge.is_small_order() {
            return false
        };
        let hash: Ed25519Scalar = eddsa_s_hash(&r_point, &pk, &msg_hash).unwrap();
        if r_point.ge
...
mod tests {
    use crate::common::{
        correct_key_proof_rho, create_hash, dh_init_secp256k1, dh_init_secp256r1, dh_init_curve25519, i2osp,
