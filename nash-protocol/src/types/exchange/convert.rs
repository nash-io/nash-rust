use exchange::model::*;
use crate as nash_protocol;
use std::convert::{TryFrom, TryInto};
use exchange::errors::MissingImplementationContent;
use std::str::FromStr;
use chrono::Utc;
use crate::protocol::subscriptions::SubscriptionResponse;
use exchange::model::websocket::{WebSocketResponse, OpenLimitsWebSocketMessage, AccountOrders, Subscription};
...
{
    type Error = Error;
    fn try_from(req: &TradeHistoryRequest) -> Result<Self> {
...
{
    type Error = Error;
    fn try_from(req: &GetHistoricRatesRequest) -> Result<Self> {
        let (before, limit, range) = try_split_paginator(req.paginator.clone())?;
        ...
        Some(paginator) => (
            ...
            match paginator.limit {
                Some(v) => Some(i64::try_from(v).map_err(|_| {
                    exchange::errors::Error::InvalidParameter(
...
{
    type Error = Error;
    fn try_from(req: &GetHistoricTradesRequest) -> Result<Self> {
        let market = req.market_pair.clone();
    ...
    fn try_from(interval: Interval) -> Result<Self> {
        match interval {
            ...
            _ => {
                ...
                Err(exchange::errors::Error::MissingImplementation(err).into())
            }
        }
    }
}
...
impl TryFrom<OrderStatus> for nash_protocol::types::OrderStatus {
    type Error = Error;
    fn try_from(status: OrderStatus) -> Result<Self> {
        Ok(match status {
            ...
            _ => {
                return Err(exchange::errors::Error::InvalidParameter(
...
impl TryFrom<OrderType> for nash_protocol::types::OrderType {
    ...
    fn try_from(order_type: OrderType) -> Result<Self> {
        match order_type {
            ...
            OrderType::Unknown => Err(exchange::errors::Error::InvalidParameter(
